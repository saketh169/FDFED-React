...
const mockOrganizationData = [
    {
        ...
        verificationStatus: {
            org_logo: 'Received',
            org_brochure: 'Pending',
            legal_document: 'Verified',
            tax_document: 'Rejected',
            address_proof: 'Received',
            business_license: 'Received',
            authorized_rep_id: 'Received',
            bank_document: 'Pending',
            finalReport: 'Not Received' // Awaiting admin report upload
        },
        fileData: { // Mock file data (Base64 is a large string placeholder)
            org_logo: { base64: 'base64_data_logo', mime: 'image/png' },
            legal_document: { base64: 'base64_data_legal', mime: 'application/pdf' },
            address_proof: { base64: 'base64_data_address', mime: 'application/pdf' }
        }
    },
    {
        _id: 'o002',
...
const OrgVerify = () => {
    ...
    const handleConfirm = (message, action) => {
        setModal({ active: true, message, onConfirm: () => {
            action();
    ...
    const fetchOrganizations = useCallback(() => {
        setOrganizations(mockOrganizationData.map((o, index) => ({...o, rowId: index + 1})));
    }, []);
    ...
    const findOrganization = (id) => organizations.find(o => o._id === id);

    const updateStatusLocally = (orgId, field, status) => {
        setOrganizations(prev => prev.map(o => {
            if (o._id === orgId) {
                return {
                    ...o,
                    verificationStatus: { ...o.verificationStatus, [field]: status }
                };
            }
            return o;
        }));
    ...
    const verifyDocument = (orgId, field) => {
        handleConfirm(`Confirm verification for ${FIELD_MAP[field].name}?`, () => {
            updateStatusLocally(orgId, field, 'Verified');
            handleNotify(`Document ${FIELD_MAP[field].name} verified.`, 'success');
        });
    };

    const rejectDocument = (orgId, field) => {
        handleConfirm(`Confirm rejection for ${FIELD_MAP[field].name}?`, () => {
            updateStatusLocally(orgId, field, 'Rejected');
            handleNotify(`Document ${FIELD_MAP[field].name} rejected.`, 'error');
        });
    };

    const finalVerify = (orgId) => {
        handleConfirm('Are you sure you want to **finally approve** this Organization?', () => {
            updateStatusLocally(orgId, 'finalReport', 'Verified');
            handleNotify('Organization has been finally approved!', 'success');
            fetchOrganizations(); 
        });
    };

    const finalReject = (orgId) => {
        handleConfirm('Are you sure you want to **finally reject** this Organization?', () => {
            updateStatusLocally(orgId, 'finalReport', 'Rejected');
            handleNotify('Organization has been finally rejected.', 'error', 5000, true);
            fetchOrganizations(); 
        });
    };

    const handleFileUpload = (orgId, file) => {
        ...
        reader.onload = (e) => {
            ...
            setOrganizations(prev => prev.map(o => {
                if (o._id === orgId) {
                    return {
                        ...o,
                        verificationStatus: { ...o.verificationStatus, finalReport: 'Received' },
                        fileData: { ...o.fileData, finalReport: { base64, mime } }
                    };
                }
                return o;
            }));
    ...
    const viewFile = (orgId, field) => {
        const organization = findOrganization(orgId);
        const file = organization?.fileData?.[field];
    ...
    const downloadFile = (orgId, field, fileName, fileExt) => {
        const organization = findOrganization(orgId);
        const file = organization?.fileData?.[field];
      ...
      <div className='w-full max-w-7xl mx-auto'>
                        ...
                        <td colSpan='2' className='p-0'>
                          <div className='bg-gray-50 p-6'>
                            <h3 className={`text-xl font-bold text-[#48BB78] mb-4 flex items-center`}>
                              ...
                              Documents Submitted by {o.org_name}
                            </h3>
        ...
        <div className='mt-8 text-center text-gray-600'>
          <p className='text-sm'>
            ...
            >
              <i className='fas fa-envelope mr-1'></i> support@orgverify.com
            </a>
            .
          </p>
        </div>
