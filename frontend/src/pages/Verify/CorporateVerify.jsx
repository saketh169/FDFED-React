...
const mockCorporateData = [
  {
    _id: 'c101',
    name: 'NutriCorp Global Inc.',
    verificationStatus: {
      business_license: 'Received',
      tax_id_document: 'Pending',
      incorporation_certificate: 'Verified',
      authorized_rep_id: 'Rejected',
      bank_account_proof: 'Received',
      financial_audit: 'Not Uploaded',
      code_of_conduct: 'Received',
      finalReport: 'Received' // Admin uploaded report, awaiting final action
    },
    fileData: {
      // Mock PDF base64 (placeholders)
      business_license: { base64: 'base64_data_biz', mime: 'application/pdf' },
      code_of_conduct: { base64: 'base64_data_code', mime: 'application/pdf' },
      // Mock PNG/Image base64 for authorized rep ID for viewer testing
      authorized_rep_id: {
        base64:
          'iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w38GIAXDSTEQxGEGAhifBpB9/1gFAAAAAElEQVQI12P4//8/w38GIAXDSTEQxGEGAhifBpB9/1gFAAAAAElEQVRIx2NgoD/AxMDGgIT/BwMXAwpOAAU/A14ABf8Ghh0wAB8AAhMB/1gFAAAAAElFTkSuQmCC', // A tiny square PNG
        mime: 'image/png'
      },
  ...
  {
    _id: 'c102',
    name: 'Wellness Partners Ltd.',
    verificationStatus: {
      business_license: 'Verified',
      tax_id_document: 'Verified',
      incorporation_certificate: 'Verified',
      authorized_rep_id: 'Verified',
      bank_account_proof: 'Verified',
      financial_audit: 'Verified',
      code_of_conduct: 'Verified',
  ...
  },
  {
    ...
    verificationStatus: {
      ...
      tax_id_document: 'Not Uploaded',
      incorporation_certificate: 'Not Uploaded',
      authorized_rep_id: 'Not Uploaded',
      bank_account_proof: 'Not Uploaded',
      financial_audit: 'Not Uploaded',
      code_of_conduct: 'Not Uploaded',
      finalReport: 'Not Received'
    }
  }
]

// Define Custom Color Variables for Tailwind (Lighter Palette)
// Header BG/Table Head: #27AE60 (Old accent, now main dark-ish)
// Accent/Button BG/Links: #48BB78 (New, brighter green)
// Dark Contrast/Hover: #1A4A40 (Kept for maximum contrast)
const PRIMARY_ACCENT = '#48BB78'
const PRIMARY_HEADER = '#27AE60'
const PRIMARY_DARK = '#1A4A40'
...
const App = () => {
  ...
  const fetchPartners = useCallback(() => {
    // Assign rowIds for local state tracking
    setPartners(mockCorporateData.map((p, index) => ({ ...p, rowId: index + 1 })))
  }, [])
  ...
  const findPartner = id => partners.find(p => p._id === id)

  const updateStatusLocally = (partnerId, field, status) => {
    setPartners(prev =>
      prev.map(p => {
        if (p._id === partnerId) {
          return {
  ...
  const verifyDocument = (partnerId, field) => {
    handleConfirm(`Confirm verification for ${FIELD_MAP[field].name}?`, () => {
      updateStatusLocally(partnerId, field, 'Verified')
      handleNotify(`Document ${FIELD_MAP[field].name} verified.`, 'success')
    })
  }

  const rejectDocument = (partnerId, field) => {
    handleConfirm(`Confirm rejection for ${FIELD_MAP[field].name}?`, () => {
      updateStatusLocally(partnerId, field, 'Rejected')
      handleNotify(`Document ${FIELD_MAP[field].name} rejected.`, 'error')
    })
  }

  const finalVerify = partnerId => {
    handleConfirm(
      'Are you sure you want to **finally approve** this Corporate Partner? This action is irreversible.',
      () => {
        updateStatusLocally(partnerId, 'finalReport', 'Verified')
  ...
  const finalReject = partnerId => {
    handleConfirm(
      'Are you sure you want to **finally reject** this Corporate Partner? This action is irreversible.',
      () => {
        updateStatusLocally(partnerId, 'finalReport', 'Rejected')
  ...
  const handleFileUpload = (partnerId, file) => {
    if (!file) return handleNotify('Please select a file to upload.', 'warning')
    
    // Only allow PDF
    if (file.type !== 'application/pdf') {
        return handleNotify('Only PDF files are allowed for the final report.', 'error');
    }
      ...
      setPartners(prev =>
        prev.map(p => {
          if (p._id === partnerId) {
            return {
              ...p,
              verificationStatus: { ...p.verificationStatus, finalReport: 'Received' },
              fileData: { ...p.fileData, finalReport: { base64, mime } }
            }
          }
          return p
        })
      )
      handleNotify('Verification report uploaded successfully. Review and finalize.', 'success')
    }
    reader.readAsDataURL(file)
  }

  const viewFile = (partnerId, field) => {
    const partner = findPartner(partnerId)
    const file = partner?.fileData?.[field]
  ...
  const downloadFile = (partnerId, field, fileName, fileExt) => {
    const partner = findPartner(partnerId)
    const file = partner?.fileData?.[field]
    ...
    <div className='min-h-screen bg-gray-50 pb-8 px-4 sm:px-6 md:px-8 font-inter'>
      ...
      <div className='w-full max-w-7xl mx-auto'> 
        <h1 className={`text-3xl sm:text-4xl font-extrabold text-[#1A4A40] text-center mb-8 pt-6`}>
          Corporate Partner Document Verification 
        </h1>
                ...
                return (
                  <React.Fragment key={p._id}>
                    <tr
                      ...
                      onClick={() => toggleDocumentDetails(p.rowId)}
                    >
                      <td className='py-4 px-6 flex items-center'>
                        <i className={`fas fa-building text-lg mr-3 text-[#48BB78]`}></i>
                        {p.name}
                      </td>
                        ...
                        <td colSpan='2' className='p-0'>
                          <div className='bg-gray-50 p-6'>
                            <h3 className={`text-xl font-bold text-[#48BB78] mb-4 flex items-center`}>
                              ...
                              Documents Submitted by {p.name}
                            </h3>
                            <div className='space-y-4'>
                              {Object.keys(FIELD_MAP).map(field => {
                                const status =
                                  p.verificationStatus[field] ||
                                  (field === 'finalReport'
                                    ? 'Not Received'
                                    : 'Not Uploaded')
                                const fileExists =
                            ...
                            <div className='mt-6 flex flex-col sm:flex-row gap-4'>
                              <button
                                ...
                                onClick={() => finalVerify(p._id)}
                                disabled={
                                  ![
                                    'Received',
                                    'Verified',
                                    'Rejected'
                                  ].includes(p.verificationStatus.finalReport)
                                }
                              >
        ...
        <div className='mt-8 text-center text-gray-600'>
          <p className='text-sm'>
            ...
            <a
              href='mailto:support@corporateverify.com'
              className={`text-[#48BB78] hover:text-[#1A4A40] transition-colors duration-200 font-semibold`}
            >
              <i className='fas fa-envelope mr-1'></i> support@corporateverify.com
            </a>
            .
          </p>
        </div>
