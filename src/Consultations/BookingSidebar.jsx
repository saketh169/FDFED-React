...
const BookingSidebar = ({
  ...
  const currentUserBookedTimesWithDietitian = useSelector(
    selectCurrentUserBookedTimesWithDietitian
  );
  ...
  const getUserConflictAt = (time) => {
    ...
    const conflict = userBookedSlots.find(
  ...
  const renderTimeSlot = (time) => {
    ...
    if (isBookedByCurrentUser) {
      ...
      isDisabled = true;
      label = (
        <span className="block text-[9px] mt-1 font-bold uppercase">Busy</span>
      );
    } else if (isSelected) {
